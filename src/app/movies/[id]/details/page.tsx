...
import { GenreLabel, VoteAverage } from "@/components";
...
const MovieDetails: FC<MovieDetailsProps> = ({ params: { id } }) => {
  const { data } = useMovieDetails(id);
  const details = data?.data;

  return (
    <div className="min-h-[calc(100vh-64px)] overflow-auto">
      {!isNil(details?.backdrop_path) && !isEmpty(details?.backdrop_path) && (
      ...
      <div className="z-10 flex flex-col gap-x-12 p-8 md:fixed md:mx-20 md:flex-row">
        {!isNil(details?.poster_path) && !isEmpty(details?.poster_path) && (
          <Image
            src={
              process.env.NEXT_PUBLIC_TMDB_BASE_IMAGE_URI +
              "w342/" +
        ...
        <section className="mt-6 md:mt-0 md:max-w-6xl md:bg-[rgb(20,20,20)] md:bg-opacity-60 md:p-12">
          <div className="mb-4 flex items-center gap-x-4">
            ...
            <VoteAverage vote={details?.vote_average} />
          </div>
